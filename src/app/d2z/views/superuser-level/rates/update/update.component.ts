  ...
  updateRecords(){
    ...
    if(selectedRows.length > 0 && this.errorMsg == null ){
      ...
      selectedRows.forEach(function(data) {
        ...
        that.updateZoneObj = (
          zoneObj={}, 
          zoneObj[rates]= that.updateRatesList, zoneObj,
          zoneObj[zoneID]= data.destinationZone != undefined ? data.destinationZone : '', zoneObj
        )
        that.updateZone.push(that.updateZoneObj);
      });
      ...
      this.updateBrokerData.push(brokerFinalObj);
      this.spinner.show();
      this.consignmenrServices.superUserAddBroker(this.updateBrokerData, (resp) => {
          this.spinner.hide();
