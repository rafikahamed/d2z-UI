...
var currentTime = new Date();
        var NonShipmentList = [];
        var fileName = '';
            fileName = "Export-NonShipment"+"-"+currentTime.toLocaleDateString();
          var options = { 
            fieldSeparator: ',',
            quoteStrings: '"',
            decimalseparator: '.',
          ...
          for (var importVal in nonshipmentSelectedRows) {
              var adminObj = nonshipmentSelectedRows[importVal];
              var importObj = (
                  importObj={},
                  importObj[reference_No]= adminObj.reference_number != null ? adminObj.reference_number: '', importObj,
                  importObj[article_Id]= adminObj.barcodelabelNumber != null ? adminObj.barcodelabelNumber.substring(18, 41) : '', importObj,
              ...
              NonShipmentList.push(importObj)
          }
        new Angular2Csv(NonShipmentList, fileName, options);        
      }else{
        this.errorMsg = "**Please select the below records to download the Export Non Shipment details";
      } 
